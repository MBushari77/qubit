...
const ProductSectionFourForm = ({ categoryId }) => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(`/productssectionfour/category/${categoryId}`);
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/productssectionfour/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/productssectionfour`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/productssectionfour/${id}`);
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      <h4 className="mb-3">Product Headlines Section</h4>
...
export default ProductSectionFourForm;
