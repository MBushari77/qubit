...
const ProductSectionTwoForm = ({ categoryId }) => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(`/productssectiontwo/category/${categoryId}`);
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/productssectiontwo/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/productssectiontwo`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/productssectiontwo/${id}`);
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      <h4 className="mb-3">Product Three Tickets Section</h4>
...
export default ProductSectionTwoForm;
