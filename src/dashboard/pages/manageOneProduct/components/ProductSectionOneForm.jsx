...
const ProductSectionOneForm = ({ categoryId }) => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(`/productssectionone/category/${categoryId}`);
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/productssectionone/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/productssectionone`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/productssectionone/${id}`);
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      <h4 className="mb-3">Product Section One</h4>
...
export default ProductSectionOneForm;
