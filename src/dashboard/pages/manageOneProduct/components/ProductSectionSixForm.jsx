...
const ProductSectionSixForm = ({ categoryId }) => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(`/productssectionsix/category/${categoryId}`);
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/productssectionsix/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/productssectionsix`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/productssectionsix/${id}`);
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      <h4 className="mb-3">Product Content Help Sector</h4>
...
export default ProductSectionSixForm;
