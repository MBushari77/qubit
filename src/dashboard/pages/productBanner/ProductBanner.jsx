...
const AddProductBanner = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      await API.post("/productbanner/add", formData);
      setForm({
        title: "",
        info: "",
        link: "",
  ...
  const fetchBanners = async () => {
    try {
      const res = await API.get("/productbanner");
  ...
  const handleDelete = async (id) => {
    try {
      await API.delete(`/productbanner/${id}`);
      fetchBanners();
    } catch (err) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await API.put(`/productbanner/${editForm.id}`, formData);
      setShowModal(false);
      fetchBanners();
    } catch (err) {
  ...
  return (
    <div className="container my-4">
      <h2 className="mb-4">Manage Products Banners</h2>
