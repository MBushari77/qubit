...
const AddHomeContentSlider = () => {
  ...
  const handleAdd = async () => {
    try {
      await API.post("/homecontentslider/add", form);
  ...
  const fetchSliders = async () => {
    try {
      const res = await API.get("/homecontentslider");
  ...
  const handleDelete = async (id) => {
    try {
      await API.delete(`/homecontentslider/${id}`);
      fetchSliders();
    } catch (err) {
  ...
  const handleUpdate = async () => {
    try {
      await API.put(`/homecontentslider/${editForm.id}`, editForm);
      setShowModal(false);
      fetchSliders();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      ...
      <div className="row">
        {sliders.map((slider) => (
          <div className="col-md-4 mb-4" key={slider.id}>
...
export default AddHomeContentSlider;
