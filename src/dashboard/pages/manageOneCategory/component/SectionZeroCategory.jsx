...
const SectionZeroCategory = ({ subCategoryId }) => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(
        `/subcategorysectionzero/category/${subCategoryId}`
      );
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/subcategorysectionzero/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/subcategorysectionzero`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/subcategorysectionzero/${id}`);
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      <h4 className="mb-3">Sub Categories Section {subCategoryId}</h4>
...
export default SectionZeroCategory;
