...
const SubCategorySectionThreeForm = ({ subCategoryId }) => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(
        `/subcategorysectionthree/category/${subCategoryId}`
      );
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/subcategorysectionthree/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/subcategorysectionthree`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/subcategorysectionthree/${id}`);
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      <h4 className="mb-3">Sub Category Grid Layout Section</h4>
...
export default SubCategorySectionThreeForm;
