...
const SubCategorySectionFiveForm = ({ subCategoryId }) => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(
        `/subcategorysectionfive/category/${subCategoryId}`
      );
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/subcategorysectionfive/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/subcategorysectionfive`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/subcategorysectionfive/${id}`);
      fetchItems();
    } catch (err) {
...
export default SubCategorySectionFiveForm;
