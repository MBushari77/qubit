...
const SubCategoryProductsForm = ({ subCategoryId }) => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(
        `/subcategoryproducts/category/${subCategoryId}`
      );
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/subcategoryproducts/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/subcategoryproducts`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/subcategoryproducts/${id}`);
      fetchItems();
    } catch (err) {
...
export default SubCategoryProductsForm;
