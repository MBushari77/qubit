...
const SubCategorySectionOneForm = ({ subCategoryId }) => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(
        `/subcategorysectionone/category/${subCategoryId}`
      );
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/subcategorysectionone/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/subcategorysectionone`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/subcategorysectionone/${id}`);
      fetchItems();
    } catch (err) {
...
export default SubCategorySectionOneForm;
