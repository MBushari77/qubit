...
const AddHeroBanner = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      await API.post("/herobanner/add", formData);
      setForm({
        title: "",
        info: "",
        link: "",
  ...
  const fetchBanners = async () => {
    try {
      const res = await API.get("/herobanner");
  ...
  const handleDelete = async (id) => {
    try {
      await API.delete(`/herobanner/${id}`);
      fetchBanners();
    } catch (err) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await API.put(`/herobanner/${editForm.id}`, formData);
      setShowModal(false);
      fetchBanners();
    } catch (err) {
  ...
  return (
    <div className="container my-4">
      <h2 className="mb-4">Manage Hero Banners</h2>
