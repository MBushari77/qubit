...
const SubCategoryBlogsForm = ({ subCategoryId }) => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(`/communitysideblogs`);
      setItems(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/communitysideblogs/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/communitysideblogs`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/communitysideblogs/${id}`);
      fetchItems();
    } catch (err) {
...
export default SubCategoryBlogsForm;
