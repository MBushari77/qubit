...
const ProjectCrew = () => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(`/projectcrew`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/projectcrew/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/projectcrew`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/projectcrew/${id}`);
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      <h4 className="mb-3">Project Crew Section</h4>
