...
const ProjectClientsLogos = () => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(`/clientslogos`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/clientslogos/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/clientslogos`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/clientslogos/${id}`);
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      <h4 className="mb-3">Project Clients Logos Section</h4>
...
export default ProjectClientsLogos;
