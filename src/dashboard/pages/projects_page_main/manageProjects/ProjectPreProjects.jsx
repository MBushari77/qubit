...
const ProjectPreProjects = () => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(`/projectpreprojects`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/projectpreprojects/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/projectpreprojects`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/projectpreprojects/${id}`);
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      <h4 className="mb-3">Previous Projects Section</h4>
...
export default ProjectPreProjects;
