...
const ProjectQuestions = () => {
  ...
  const fetchItems = async () => {
    try {
      const res = await API.get(`/projectquestions`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        await API.put(`/projectquestions/${form.id}`, formData);
        alert("Item updated");
      } else {
        await API.post(`/projectquestions`, formData);
        alert("Item added");
      }
      setModalOpen(false);
      fetchItems();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`/projectquestions/${id}`);
      fetchItems();
    } catch (err) {
  ...
  return (
    <div className="container py-4">
      <h4 className="mb-3">Project Questions Section</h4>
